...
Boolean DMA_IsChannelEnable(DMA_TypeDef* DMA_Channelx)
{
	uint32_t CheckBit;
	if (DMA_Channel_0 == DMA_Channelx)
	{
		CheckBit = 0x0001;
	} 
	else if (DMA_Channel_1 == DMA_Channelx)
	{
		CheckBit = 0x0002;
	}
	else if (DMA_Channel_2 == DMA_Channelx)
	{
		CheckBit = 0x0004;
	}
	else if (DMA_Channel_3 == DMA_Channelx)
	{
		CheckBit = 0x0008;
	}

	if(DMA->ChEnReg_L & CheckBit)
		return TRUE;
	else
		return FALSE;
}
