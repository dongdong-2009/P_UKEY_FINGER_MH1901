...
unsigned char G_XFA_Data[1024*2];
                           ...
                           uint8_t *params)
{
  ...
  static unsigned short slength = 0;
  unsigned char cmd[6];
  ...
  case 0xFE:
	...
	if(BOT_IDLE==MSC_BOT_State)
	{
		//Ω” ’÷∏¡Ó
		slength = 0;
		slength =  (MSC_BOT_cbw.CB[8] +(MSC_BOT_cbw.CB[7] << 8)) ;	
		MSC_BOT_State = BOT_DATA_OUT;  
		DCD_EP_PrepareRx(pdev, MSC_OUT_EP,
						(uint8_t *)g_b_MassComm, 
						slength);
		MSC_BOT_DataLen =0;
...
//		g_APDU.pOutData=g_b_MassComm+8*1024;
		g_APDU.pOutData=g_b_MassComm+1024;
...
//				  g_reboot=4;
			  ...
			  MSC_BOT_DataLen = g_APDU.RLE+2;
			  memcpy(MSC_BOT_Data,g_APDU.pOutData,MSC_BOT_DataLen);
