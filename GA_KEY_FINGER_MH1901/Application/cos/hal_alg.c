								...
								unsigned char *pbout)
{
	...
	if(cbc == 0)
	{
		if(encrypt==0)
		{
			mh_sm4_enc(ECB,pbout,len,pbin,len,pkey,piv,mh_rand_p,NULL); 	
		}	
		else
		{
			mh_sm4_dec(ECB,pbout,len,pbin,len,pkey,piv,mh_rand_p,NULL);
		}
	}
	else
	{
		if(encrypt==0)
		{
			mh_sm4_enc(CBC,pbout,len,pbin,len,pkey,piv,mh_rand_p,NULL); 	
		}	
		else
		{
			mh_sm4_dec(CBC,pbout,len,pbin,len,pkey,piv,mh_rand_p,NULL);
		}
	}
								...
								unsigned char *pbout)
{
	...
	mh_tdes_key_def KEY;
	memcpy(KEY.k1,pkey,8);
	memcpy(KEY.k2,pkey+8,8);
	memcpy(KEY.k3,pkey,8);
	...
	if(cbc == 0)
	{
		if(encrypt==0)
		{
			mh_tdes_enc(ECB,pbout,len,pbin,len,&KEY,piv,mh_rand_p,NULL); 	
		}	
		else
		{
			mh_tdes_dec(ECB,pbout,len,pbin,len,&KEY,piv,mh_rand_p,NULL);
		}
	}
	else
	{
		if(encrypt==0)
		{
			mh_tdes_enc(CBC,pbout,len,pbin,len,&KEY,piv,mh_rand_p,NULL); 	
		}	
		else
		{
			mh_tdes_dec(CBC,pbout,len,pbin,len,&KEY,piv,mh_rand_p,NULL);
		}
	}
	return 0;
}
